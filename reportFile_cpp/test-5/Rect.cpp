...
#include "Rect.h"
/*---------------------------------------------------------*/
Rect::Rect(int rows, int cols) {  //构造函数
...
/*---------------------------------------------------------*/
Rect Rect::operator+(const Rect &rect) { //重载了+号运算符的实现
...
Rect Rect::operator-(const Rect &rect) { //重载了-号运算符的实现
  assert(this->rows == rect.rows &&
         this->cols == rect.cols); //断言函数，可以保证矩阵的大小相等
  Rect *rectTemp = new Rect(rect.rows, rect.cols);
  for (int i = 0; i < rect.rows; i++) {
    for (int j = 0; j < rect.cols; j++) {
      rectTemp->head[i][j] = rect.head[i][j] - this->head[i][j];
    }
...
/*----------------------------------------------------------*/
Rect::Rect(const Rect &rectTemp) { //用深拷贝的方式来初始化一个矩阵的构造函数
...
/*----------------------------------------------------------*/
void Rect::show() { // show函数，用于展示打印的函数
...
/*----------------------------------------------------------*/
void Rect::input() { // input函数，用来cin输入更改的函数
...
/*----------------------------------------------------------*/
void Rect::find(int x, int y) { // find查询函数，用来查询对应的下标的值
